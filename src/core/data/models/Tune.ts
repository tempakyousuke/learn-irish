 ...
 */
export interface TuneFull
	extends TuneBase,
		TuneMusicalProperties,
		TuneMetadata,
		TuneExternalLinks {}
...
export function createTune(
	base: TuneBase,
	properties: Partial<Omit<TuneFull, keyof TuneBase>> = {}
): TuneFull {
	return {
		...base,
		...properties
...
export function isValidTune(tune: Partial<TuneFull>): tune is TuneFull {
	return (
		typeof tune.id === 'string' && typeof tune.tuneNo === 'number' && typeof tune.name === 'string'
	);
}
