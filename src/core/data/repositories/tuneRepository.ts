...
export class TuneRepository {
  ...
  private static readonly tunesCache = createCache<TuneFull[]>('tunes', TuneRepository.CACHE_EXPIRY);
  ...
  public static async getTuneById(id: string): Promise<TuneFull | null> {
    try {
      ...
      throw new Error(`曲データの取得に失敗しました: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
