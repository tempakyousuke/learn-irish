<script lang="ts">
	...
	$: rows = sheetText.trim().split('\n').filter(row => row.trim());
	$: parsedTunes = rows.map(row => {
	...
	const saveTunes = async () => {
		...
		try {
			const promises = parsedTunes.map(tune => 
				addDoc(collection(db, 'tunes'), tune)
			);
				...
				<table class="min-w-full divide-y divide-gray-200">
					<thead class="bg-gray-50 sticky top-0">
						<tr>
							...
							<th class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Set</th>
							...
							<th class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Genre</th>
							<th class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rhythm</th>
							<th class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Key</th>
							<th class="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Difficulty</th>
					...
					<tbody class="bg-white divide-y divide-gray-200">
						{#each parsedTunes as tune, index}
							<tr class="hover:bg-gray-50">
								...
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{tune.setNo || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-900">{tune.tuneNo || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm font-medium text-gray-900">{tune.name || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">{tune.genre || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">{tune.rhythm || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">{tune.key || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">{tune.difficulty || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">{tune.date || '-'}</td>
								<td class="px-2 py-2 whitespace-nowrap text-sm text-gray-500">
									{#if tune.link}
										<a href={tune.link} target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-900">
											Link
										</a>
									{:else}
										-
